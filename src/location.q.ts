...
window.location.q = document.location.q = function (key: any = null, val, push) {
      ...
      if (typeof key == "string" && typeof val === "undefined") return qOut[key]
         ...
         const process = (obj: any) => {
            ...
            for (let par in obj) {
               ...
               else if (obj[par] !== null) out.push([par, encodeURIComponent(obj[par])])
            }
   ...
   if (updated !== false) {
      ...
      if (push && typeof window.history.pushState === "function") window.history.pushState({
         path: newurl
      }, '', newurl)
